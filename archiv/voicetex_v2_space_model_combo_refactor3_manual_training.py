...
class VoicetexApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Voicetex v2 – refactor 2: safe paste + safe auto-training")
        ...
        # Sor/bekezdés előtag (Space gomb között rögzítések közt)
        self._sor_elotag      = ""    # "", "\n" vagy "\n\n"
        self._space_szamlalo  = 0     # hányszor nyomták a Space-t
        self._kozotti_allapot = False # True = hotkey_up és hotkey_down között
        self._space_hook      = None  # keyboard hook a Space-re
    ...
    def _auto_tanitas_thread(self, koteg):
        ...
        try:
            ...
            lora_merge_es_ct2_konvertalas(self.log_status)
    ...
    def _atiras_srt(self, wav_ut, srt_ut, nyelv="en"):
        ...
        try:
            ...
            with self.model_lock:
                szegmensek_gen, info = self.fw_model.transcribe(
            ...
            kuszob = self.felirat_parbeszed_var.get()
    ...
    def run_inference(self):
        ...
        try:
            ...
            # Hallucináció szűrő
            if any(b in text_result.lower() for b in ["amara","felirat","subtitle","közösség"]):
                text_result = ""
            ...
            def _paste(szoveg=text_result):
                # Sor/bekezdés előtag hozzáadása ha volt Space a rögzítések közt
                elotag = self._sor_elotag
                self._sor_elotag     = ""
                self._space_szamlalo = 0
                if elotag and szoveg.strip():
                    szoveg = elotag + szoveg
                paste_text_for_ui = szoveg if szoveg.strip() else "[Nem értett semmit]"
                autopaste_kell = self.auto_paste_var.get() and KEYBOARD_ELERHETO and szoveg.strip()
    ...
    def _hotkey_up(self, event):
        if self.is_recording:
            # A korábbi verzió itt globális Space-hookot tett fel
            # suppress=True beállítással. Ez Windows alatt elnyelte a szóközt
            # más programokban is, amíg újra nem indult a következő PTT-felvétel.
            # Ezért a Space figyelését kikapcsoljuk: a szóköz innentől normálisan működik.
            self._kozotti_allapot = False
            self._space_szamlalo  = 0
            self._sor_elotag      = ""
            if self._space_hook is not None:
    ...
    def _space_pressed(self, event):
        """Space gomb figyelő – csak rögzítések között aktív."""
        if not self._kozotti_allapot:
            return
        if event.event_type == "down":
            self._space_szamlalo += 1
            if self._space_szamlalo == 1:
                self._sor_elotag = "\n"
            elif self._space_szamlalo >= 2:
                self._sor_elotag = "\n\n"
    ...
    def _batch_thread(self, wave_path, txt_path):
        try:
            ...
            for i in range(n):
                ...
                segs, _ = self.fw_model.transcribe(szelet_ut, language="hu",
                                                   beam_size=3, vad_filter=True)
                tipp = " ".join(s.text.strip() for s in segs).strip()
                if any(b in tipp.lower() for b in ["amara","felirat","subtitle","közösség"]):
                    tipp = ""
                ...
                lora_automatikus_mentes()
                lora_merge_es_ct2_konvertalas(self.log_to_factory)
                self.load_active_model()
